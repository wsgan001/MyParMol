...
public class DataBase implements EdgeNodeRelabler{
    ...
    /**
     * creates a new sorted, cleandup and renamed Dataset as expected in graphSet_Projection
     * @param graphs
     * @param minFreq
     * @param fragments
     * @param factory
     */
    public DataBase(Collection graphs, float[] minFreq, FragmentSet fragments, GraphFactory factory){
     ...
     * @param graphs
     * @param edges the map for the edgelabel counts
     * @param nodes the map for the nodelabel counts
     */
    ...
    private class Tuple implements Comparable{
        ...
        /**
         * creates a new Tuple
         * @param c count of the given label l
         * @param l 
         */
        public Tuple(int c,int l){ this.count=c;this.label=l; }
    ...
    /**
     * creates a node Relabler for frequent node labels and add single noded frequent Graphs to fragments
     * @param nodes map of nodelabel counts
     * @param minFreq
     * @param fragments the set for frequent graphs
...
//				create single noded frequent Graph                  
                ...
                fragments.add(new FrequentFragment(ng,s,frequencies));
//              ---
            }
        }
    ...
    /**
     * @param edges map of edgelable counts
     * @param minFreq
    ...
    /**
     * build for each graph in the Collection a corresponding GSpanGraph 
     * and builds an map between edges an containing GSpanGraph
     * @param graphs the set of graphs
     * @param nodeR for renaming nodes
     * @param edgeR  for renaming edges
     * @return the map 
     */
    private Map getAllEdges(Collection graphs, Relabler nodeR, Relabler edgeR){
        ...
        for (Iterator git=graphs.iterator();git.hasNext();){
            ...
            if (maxNodeCount<nodeCount) maxNodeCount=nodeCount;
            if (gg.getEdgeCount()==0) size--;
            for (Iterator eit=gg.edgeIterator();eit.hasNext();){
                GSpanEdge e=(GSpanEdge) eit.next();
                GraphSet s=(GraphSet) map.get(e);
                if (s==null) { s=new GraphSet(); map.put(e,s); }
                s.add(gg);
            }
        }
        return map;
    ...
    public Iterator frequentEdges(){
        return new Iterator(){
            GSpanEdge[] edgeArray=(GSpanEdge[]) sortedFrequentGSpanEdges.toArray(new GSpanEdge[sortedFrequentGSpanEdges.size()]);
            int pos=0;
            ...
            public void remove(){
                ...
                if (edge==null) throw new NoSuchElementException("element always deleted");
                for (Iterator ggit=getContainingGraphs(edge).iterator();ggit.hasNext();){
    ...
    public int getRealEdgeLabel(int edge){ return edgeLabels[edge]; } 
    public int getRealNodeLabel(int node){ return nodeLabels[node]; }
    public int getEdgeLabel(int edge) { return edgeRelabler.getLabel(edge); }
    public int getNodeLabel(int node) { return nodeRelabler.getLabel(node); }
