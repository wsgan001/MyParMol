...
/**
 * This class represents graphs that are classified, directed, hold information about existing rings and use an adjacency matrix.
 * 
 * @author Thorsten Meinl <Thorsten.Meinl@informatik.uni-erlangen.de>
 */
public class DirectedMatrixRingGraph extends ClassifiedDirectedMatrixGraph implements RingGraph {
	protected int[] m_edgeRingMembership;
	
	/**
	 * Creates a new empty DirectedMatrixRingGraph with the given class frequencies.
	 * 
	 * @param classFrequencies the frequencies of this graph in the different classes
	 */
	public DirectedMatrixRingGraph(float[] classFrequencies) {
	...
	/**
	 * Creates a new empty DirectedMatrixRingGraph with the given id and class frequencies. 
	 * 
	 * @param id an id for the new graph
	 * @param classFrequencies the frequencies of this graph in the different classes
	 */
	public DirectedMatrixRingGraph(String id, float[] classFrequencies) {
	...
	/**
	 * Creates a new DirectedMatrixRingGraph that is a copy of the given template graph.
	 * 
	 * @param template the graph that should be copied
	 */
	public DirectedMatrixRingGraph(UndirectedMatrixRingGraph template) {
		super(template);
	...
	/*
	 *  (non-Javadoc)
	 * @see de.parmol.graph.MatrixGraph#resizeGraph(int, int)
	 */
	...
	/**
	 * This factory creates new DirectedMatrixRingGraphs.
	 * 
	 * @author Thorsten Meinl <Thorsten.Meinl@informatik.uni-erlangen.de>
	 */
	public static class Factory extends ClassifiedUndirectedMatrixGraph.Factory {
		...
		public MutableGraph createGraph() {
			return new DirectedMatrixRingGraph(DEFAULT_CLASS);
		}
		public ClassifiedGraph createGraph(float[] classFrequencies) {
			return new DirectedMatrixRingGraph(classFrequencies);
		}
		public ClassifiedGraph createGraph(String id, float[] classFrequencies) {
			if (id != null) return new DirectedMatrixRingGraph(id, classFrequencies);
			return new DirectedMatrixRingGraph(classFrequencies);
		}
		public MutableGraph createGraph(String id) {
			if (id != null) return new DirectedMatrixRingGraph(id, DEFAULT_CLASS);
			return new DirectedMatrixRingGraph(DEFAULT_CLASS);
		}
	}
